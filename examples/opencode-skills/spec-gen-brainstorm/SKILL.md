...
Then probe the structural index by calling the spec-gen MCP tool `orient` with:
```json
{
...
Call the spec-gen MCP tool `get_architecture_overview` with `{"directory": "$PROJECT_ROOT"}`.

Note hub functions and cross-domain dependencies in `$DOMAINS_AFFECTED`.

### Step 3 — Generate change proposal

Call the spec-gen MCP tool `generate_change_proposal` with:
```json
{
  "directory": "$PROJECT_ROOT",
...
Call the spec-gen MCP tool `annotate_story` with:
```json
{
  "directory": "$PROJECT_ROOT",
